...
func (c *Component) initService(ctx context.Context) error {
	...
	// start kafka consumer logging go-routines
...
func (c *Component) drainTopic(ctx context.Context, topic, group string, wg *sync.WaitGroup) error {
	...
	waitUntilDrained := func() {
		...
		log.Info(ctx, "drain consumer closed")
	}
...
func (c *Component) Close() {
	...
	wg := &sync.WaitGroup{}
	if err := c.drainTopic(c.ctx, c.cfg.KafkaConfig.CategoryDimensionImportTopic, ComponentTestGroup, wg); err != nil {
	...
	if err := c.drainTopic(c.ctx, c.cfg.KafkaConfig.InstanceStartedTopic, c.cfg.KafkaConfig.InstanceStartedGroup, wg); err != nil {
		log.Error(c.ctx, "error draining topic", err, log.Data{
			"topic": c.cfg.KafkaConfig.InstanceStartedTopic,
			"group": c.cfg.KafkaConfig.InstanceStartedGroup,
		})
	}
	wg.Wait()
}
